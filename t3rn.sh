...
set_private_key() {
    while true; do
        ...
        if [ ${#PRIVATE_KEY_LOCAL} -eq 64 ]; then
            ...
            echo "Invalid private key. It must be 64 characters long."
        fi
    done
}
...
start_service() {
    ...
    echo "You can check the services status using: sudo systemctl status executor.service"
}

display_log() {
    echo "Display log from the executor service:"
